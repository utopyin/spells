// @ts-nocheck
/*
 * Vendored copy of upstream test fixture data from
 * https://github.com/kaelzhang/node-ignore/blob/7.0.5/test/fixtures/cases.js
 *
 * The `cases` array below is byte-identical to upstream. The surrounding
 * runner glue (tap env vars, debug logger, win32 symbol setup) has been
 * trimmed away since we run these through vitest instead of tap.
 */
import fs from "node:fs";
import path from "node:path";

const __dirname = new URL("./ignore-fixtures/", import.meta.url).pathname;
...
const real_cases = cases_to_test_only.length ? cases_to_test_only : cases;

export const iterateCases = (iteratee) => {
...
export { IS_WINDOWS, SHOULD_TEST_WINDOWS };
